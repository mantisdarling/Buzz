      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-amber-500/20 pb-6">
          <div>
            ...
            <p className="text-xs text-slate-300 font-light mt-1">
              Review verified claims, inspect multi-model breakdowns, and rate accuracy
            </p>
          </div>
        </div>
        ...
        {loading ? (
          <div className="p-16 text-center">
            ...
            <p className="text-xs text-slate-400 mt-3 font-light">Loading archive records...</p>
          </div>
        ) : error ? (
          ...
          <div className="p-16 text-center rounded-3xl luxury-glass-card border border-amber-500/20 space-y-3">
            <FileText className="w-10 h-10 text-amber-400/50 mx-auto" />
            <h3 className="text-base font-serif font-bold text-slate-100">No Submissions Found</h3>
            <p className="text-xs text-slate-400 font-light max-w-sm mx-auto">
              You haven&apos;t analyzed any claims or URLs yet. Head over to the Analyzer to submit your first claim.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {items.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="rounded-2xl luxury-glass-card border border-amber-500/20 overflow-hidden shadow-lg transition duration-200 hover:border-amber-400/40"
                >
                  {/* Header Row */}
                  <div
                  ...
                  >
                    <div className="flex items-center space-x-4 min-w-0 flex-1">
                      <div className="p-2.5 rounded-xl bg-slate-900 border border-amber-500/20 text-amber-400">
                  ...
                  {/* Expanded Body */}
                  {isExpanded && (
                    <div className="p-6 border-t border-amber-500/20 bg-slate-950/60 space-y-6">
                      {item.scores && (
                        <VerdictChart
                          scores={item.scores}
                      ...
                      <div className="flex items-center justify-between pt-2">
                        <span className="text-xs text-slate-400 font-light">Rate determination accuracy:</span>
                        <div className="flex items-center space-x-2">
                          <button
                            onClick={() => handleFeedback(item.id, true)}
                            className={`flex items-center space-x-1.5 px-3.5 py-1.5 rounded-xl text-xs font-semibold border ${
